...
contract Helper is PheasantNetworkBridgeChild {
    ...
    function setUpDestHashes(
        address user,
        uint256 index,
        bytes32 txHash
    ) public {
        destTxHashes[user][index] = txHash;
    }
    ...
    function getParamsAddress() public view returns (address) {
        return address(params);
    }

    function slashForBondManagerTest(address _bondManager, uint8 _tokenIndex, uint256 _amount) public {
        IBondManager(_bondManager).slash(_tokenIndex, _amount);
    }

    function getToAndValueFromTransferDataHelper(uint8 tokenTypeIndex, bytes calldata transaction) external view returns (address, uint256) {
        return getToAndValueFromTransferData(tokenTypeIndex, transaction);
    }

    function setRelayerFeeHelper(uint8 _tokenTypeIndex) public {
        relayerFeeHelper = params.getRelayerFee(_tokenTypeIndex);
    }
