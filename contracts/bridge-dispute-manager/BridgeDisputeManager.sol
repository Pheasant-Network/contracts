...
interface IChildCheckpointManager {
...
contract BridgeDisputeManager {
    ...
    /******** Variables ********/
    IChildCheckpointManager internal checkpointManager;
    /******** Constant ********/
    ...
    constructor(address _checkpointManager) {
        require(_checkpointManager != address(0), "Invalid checkpoint manager address");
        checkpointManager = IChildCheckpointManager(_checkpointManager);
    }
    ...
    /**
     * @dev Verifies the block hash with specific block number.
     * @param _blockHash The block hash to verify.
     * @param _blockNumber The block number of L1 to verify.
     * @return True if the blockhash matches given blocknumber, false otherwise.
     */
